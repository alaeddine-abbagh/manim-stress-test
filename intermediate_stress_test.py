...
class IntermediateStressTest(Scene):
    ...
    def construct(self):
        ...
        if test_mode:
            # Fast test mode - reduced complexity
            title = Text("Intermediate Test (FAST)", font_size=48, color=ORANGE)
            title.to_edge(UP)
            self.play(Write(title), run_time=0.5)
            self.wait(0.3)
            
            # Create fewer particles for fast test
            particles = VGroup()
            for i in range(12):  # Reduced from 80 to 12
                particle = Dot(radius=0.08, color=random_color())
                angle = i * 2 * PI / 12
                radius = 2 * np.sqrt(np.random.random())
        ...
        if test_mode:
            ...
            # Quick rotation and scaling
            self.play(
                Rotate(particles, PI/4, run_time=1),
                particles.animate.scale(1.2),
            )
            self.play(
                particles.animate.scale(0.9),
                run_time=0.5
            )
        else:
            self.play(
        ...
        if not test_mode:
            # Create a spiral motion (skip in test mode)
            self.play(
        ...
        # Mathematical visualization (simplified in test mode)
        self.play(FadeOut(particles), run_time=0.5 if test_mode else 1)
        
        if test_mode:
            # Simple mathematical plot for fast test
            axes = Axes(
                x_range=[-2, 2, 1],
                y_range=[-1, 1, 1],
            ...
            simple_func = axes.plot(lambda x: np.sin(2*x), color=RED)
            
            self.play(Create(axes), run_time=0.5)
            self.play(Create(simple_func), run_time=1)
            self.wait(0.3)
        else:
            # Full mathematical visualization: Fourier series
            axes = Axes(
                x_range=[-4, 4, 1],
                y_range=[-3, 3, 1],
                axis_config={"color": BLUE},
        ...
        if test_mode:
            # Skip 3D visualization in test mode - go directly to final message
            self.play(FadeOut(Group(*self.mobjects)), run_time=0.5)
            final_text = Text("Intermediate Test Complete!", font_size=36, color=GREEN)
